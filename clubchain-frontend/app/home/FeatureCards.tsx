...
import { Calendar, Zap, DollarSign, CheckCircle2, Trophy } from "lucide-react";
...
const FeatureCards = memo(function FeatureCards() {
  const features = [
    ...
    },
    {
      ...
      description: "Track club performance with dynamic rankings and leaderboard systems",
      badge: "Coming Soon",
        ...
        return (
          <ScrollSection key={feature.title} direction="up" delay={index * 150}>
            <Card hover className="h-full card-interactive group relative overflow-hidden border-2 hover:border-primary/50 transition-all shadow-elevation-2 hover:shadow-elevation-3">
              ...
              {/* Badge */}
              <div className={`absolute top-3 right-3 px-3 py-1.5 rounded-full text-xs font-bold border ${feature.badgeColor} backdrop-blur-sm z-20 shadow-sm`}>
                {feature.badge}
              </div>
              
              <CardBody className="text-center relative z-10 pt-12 pb-8 px-8 flex flex-col h-full">
                ...
                <p className="text-text-muted leading-relaxed flex-grow mb-6">{feature.description}</p>
